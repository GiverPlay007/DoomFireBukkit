...
public class GifHolder extends Image implements ConfigurationSerializable {
	...
	public GifHolder(Map<String, Object> data) {
		...
		temp.setI(Bukkit.getScheduler().scheduleSyncRepeatingTask(Game.getInstance(), new Runnable() {
			public void run() {
		...
		temp2.setI(Bukkit.getScheduler().scheduleSyncRepeatingTask(Game.getInstance(), new Runnable() {
			public void run() {
	...
	public void init() {
		...
		}.runTaskLaterAsynchronously(Game.getInstance(), 0);
	}
	...
	public void createFrames(final File gif, final int height, UUID owner) {
		Bukkit.getScheduler().scheduleAsyncDelayedTask(Game.getInstance(), new Runnable() {
			public void run() {
